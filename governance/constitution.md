...
6. 立法を選んだ議題は、条文および執行定義を公開したうえ、成立判定によって確定する。成立判定は、実行規則の定めるところにより、独立した複数席の必要票または公開の記名投票のいずれかによる。
7. 成立判定を開始した後に、条文または執行定義を変更してはならない。
8. 記名投票による場合、投票は実行規則で定める期間の満了によって締め切る。実行規則が定める場合において、受付時に固定された有権者の全員が投票を終えたときは、期間の満了を待たずに開票する。この場合、締切前に認められる投票の変更は、その時点で終了する。
9. 複数席の必要票によって成立させる場合は、次の各号による。
   1. 席は、公開された条文案、公開の討論、現行憲法および現行法だけを根拠とし、理由を公開しなければならない。
   2. 法律は、施行後も、実行規則で定める数の構成員が停止を求めたときは直ちに停止する。停止中の法律は執行できず、次の国会で維持または廃止を決める。
   3. 憲法改正は、成立判定の後、実行規則で定める保留期間を置く。保留期間中に同規則で定める数の構成員が異議を出したときは成立しない。
   4. 前二号の制動を欠く実行規則によって、席の必要票だけで成立させてはならない。
10. 国会は、議題ごとに、選んだ結論、その理由、参照した討論および適用した現行法を公開しなければならない。継続審議とするときは、次の国会までに構成員へ求める論点を示さなければならない。
11. 開会の間隔、1回の国会で扱う議題数の上限、公開記録から議題を発見するかどうか、合議の席数と必要票、継続審議の上限、成立判定の方法と必要票、停止の必要数、保留期間、投票範囲、定足数および成立条件は、本憲法末尾の実行規則で定める。運営者はこれらを変更できず、変更には改正手続を要する。
12. すべての成立判定、停止、異議および特別な承認権限の行使は、監査可能な形で記録されなければならない。
...
1. 本憲法および本憲法に含まれる実行規則は、第八条の国会の手続を経て確定した改正案について、同条の成立判定を経なければ改正できない。
2. 改正の必要票、定足数、成立条件および保留期間は、改正時点で有効な実行規則に従う。
